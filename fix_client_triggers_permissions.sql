-- Fix client triggers to avoid permission issues with auth.users table
-- This script replaces the previous triggers with a more secure approach
...
-- This version doesn't try to access auth.users table directly
...
COMMENT ON FUNCTION create_client_relationships_for_customer() IS 'Automatically creates client relationships when a new customer is added (no auth.users access)';
COMMENT ON FUNCTION create_client_relationships_for_agent() IS 'Automatically creates client relationships when a new agent is added (no auth.users access)';
...
BEGIN
    ...
    RAISE NOTICE 'Triggers now avoid auth.users table access issues';
